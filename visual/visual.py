...
class Visual:
    def __init__(
        ...
        self.zoom_speed: int = 10
        self.camera_speed: float = 0.2
        ...
        self.loop: bool = True
        self.drone_list: list[Drone] = drones
        self.stars: list[list[float]] = []
        self.hue: float = 0.0
        self.animation_turn: int = 0
        self.animation_progress: float = 0.0
        self.history: list[list[tuple[str, Hub, Hub]]] = (
            simulation.history if simulation else []
        )

        for _ in range(100):
            x: float = float(random.randint(0, self.width))
            y: float = float(random.randint(0, self.height))
            phase: float = random.uniform(0, math.pi * 2)
    ...
    def _load_images(self) -> None:
        ...
        self.planet_original: pygame.Surface = pygame.transform.scale(
            pygame.image.load("visual/planetfinal.png").convert_alpha(),
            (90, 60),
        )
        self.start_end_img: pygame.Surface = pygame.transform.scale(
            pygame.image.load("visual/start_endbg.png").convert_alpha(),
            (100, 70),
        )
        self.yellow_img: pygame.Surface = pygame.transform.scale(
            pygame.image.load("visual/planetyellow.png").convert_alpha(),
            (90, 60),
        )
        self.gray_img: pygame.Surface = pygame.transform.scale(
            pygame.image.load("visual/planetgray.png").convert_alpha(),
            (90, 60),
        )
        self.drone_img: pygame.Surface = pygame.transform.scale(
            pygame.image.load("visual/drone.png").convert_alpha(),
            (90, 60),
        )
        self.cyan_img: pygame.Surface = pygame.transform.scale(
            pygame.image.load("visual/cyanplanet.png").convert_alpha(),
            (90, 60),
        )
        self.brown_image: pygame.Surface = pygame.transform.scale(
    ...
    def map_to_screen(self, x: float, y: float) -> tuple[float, float]:
        ...
        screen_x: float = screen_center_x + (x - self.camera_x) * self.zoom
        screen_y: float = screen_center_y + (y - self.camera_y) * self.zoom
    ...
    def draw_stars(self) -> None:
        for star in self.stars:
            ...
            size: int = 1 if brightness < 0.7 else 2
            pygame.draw.circle(
                self.screen, (255, 255, 255), (int(x), int(y)), size
            )
            star[2] += 0.10 * speed
        ...
        for hub in self.hubs:
            ...
            if hub == self.start_hub or hub == self.end_hub:
                ...
                rect = planet.get_rect(
                    center=(int(x + 45), int(y + 30 + offset_y))
                )
            ...
            rect = planet.get_rect(center=(int(x + 45), int(y + 30)))
            self.screen.blit(planet, rect)
